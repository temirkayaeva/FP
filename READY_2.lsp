...
; 7. Определите фильтр (УДАЛИТЬ-ЕСЛИ-НЕ пред список), удаляющий из списка список все элементы, которые не обладают 
; свойством, наличие которого проверяет предикат пред.
...
(print (удалить-если-не (lambda (x) (= x 1)) '(1 1 2 1 0 1)))
...
(defun многофун (список-функций аргумент)
  (mapcar #'(lambda (функция) (funcall функция аргумент)) список-функций))

(print (многофун '(cadr cddr car) '(12 4 5 6 7 8 9 0)))
